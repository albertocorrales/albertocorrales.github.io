...
If the current node is a Leader node, we need to reduce its priority and force a re-election until the node has the role of `Follower`. You can find more information about the leader resignation process [in this link](https://www.eventstore.com/blog/event-store-20.6.0-release).
